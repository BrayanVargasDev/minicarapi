...
class ServicesService {
  ...
  async generatePdfReport(userId, dataCallback, endCallback, isAdmin = false) {
    const sequelize = models.Service.sequelize;
    const doc = new pdf({
      bufferPages: true
    });
    ...
    const query = `
      ...
      ${isAdmin ? '' : 'WHERE sch.customer_id = :id'}
      ${isAdmin ? '' : "AND s.current_date >= CURRENT_DATE - INTERVAL '30 days'"}
      ORDER BY num ASC
    `;

    const person = await models.People.findByPk(userId);
    const queryOptions = {
      type: sequelize.QueryTypes.SELECT,
    }

    if (!isAdmin) {
      queryOptions.replacements = { id: userId }
    }

    doc.on('data', dataCallback);
    doc.on('end', endCallback);

    let servicios = await sequelize.query(query, queryOptions);
    ...
    doc.setDocumentHeader({
      ...
      doc.text(`Documento: ${person.document}`, {
        width: 420,
        align: 'left'
      })
      doc.text(`Nombre: ${person.name} ${person.lastName ? person.lastName : ''}`, {
        width: 420,
        align: 'left'
      })
