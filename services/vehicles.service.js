...
class VehiclesService {
  ...
  async find(limit, offset, userId, role = null) {
    let response;
  ...
  async update(id, changes) {
    ...
    if (lastService && changes.mileage && changes.mileage >= lastService.coming_mileage) {
      notificationFlag = true;
      notificationType = 'scheduleMileage';
    }

    if (lastService && lastService.coming_date && new Date() >= new Date(lastService.coming_date)) {
      notificationFlag = true;
